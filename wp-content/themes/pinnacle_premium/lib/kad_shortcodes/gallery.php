                ...
                ?>
                    <div class="<?php echo esc_attr($itemsize); ?> gallery_item">
                      <div class="carousel_item grid_item">
                        <?php if($link != 'none') { ?>
                            <a href="<?php echo esc_attr($attachment_url);?>" <?php echo $lightbox_data;?> class="lightboxhover">
                        <?php } ?>
                            <img src="<?php echo esc_url($image[0]);?>" width="<?php echo esc_attr($image[1]); ?>" height="<?php echo esc_attr($image[2]); ?>" <?php echo $img_srcset_output; ?> alt="<?php echo esc_attr($attachment->post_excerpt); ?>" data-grid-id="<?php echo esc_attr($gid);?>" class="kt-gallery-img" />
                             <?php 
          ...
          ob_start(); ?>
                <div id="flexslider<?php echo $gallery_id;?>" class="flexslider loading kt-flexslider" style="max-width:<?php echo esc_attr($width);?>px;" data-flex-speed="<?php echo esc_attr($speed); ?>" data-flex-anim-speed="<?php echo esc_attr($transpeed); ?>" data-flex-animation="fade" data-flex-auto="<?php echo esc_attr($autoplay); ?>">
                   ...
                   <?php foreach ($attachments as $id => $attachment) {
                            $attachment_src = wp_get_attachment_image_src($id, 'full');
                            $attachment_url = $attachment_src[0];
                            $image = aq_resize($attachment_url, $width, $height, true, false, false, $attachment);
                            if(empty($image[0])) {$image = array($attachment_url,$attachment_src[1],$attachment_src[2]);}
                            $img_srcset_output = kt_get_srcset_output( $image[1], $image[2], $attachment_url, $id);
                            if($lightboxsize != 'full') {
                                $attachment_url = wp_get_attachment_image_src( $id, $lightboxsize);
                            ...
                            echo '<li><a href="'.esc_attr($attachment_url).'" '.$lightbox_data.' class="lightboxhover">';
                                echo '<img src="'.esc_url($image[0]).'" width="'.esc_attr($image[1]).'" height="'.esc_attr($image[2]).'" '.$img_srcset_output.' alt="'.esc_attr($attachment->post_excerpt).'" class=""/>';
                                    if (trim($attachment->post_excerpt) && $caption == 'true') {
  ...
  <?php  $output = ob_get_contents();
    ...
    $output .= '</div>';
        ...
        foreach ($attachments as $id => $attachment) {
            ...
            $output .= '<div class="kt-intrinsic" style="padding-bottom:'.$paddingbtn.'%;"><img '.$image_src_output.' width="'.esc_attr($image[1]).'" height="'.esc_attr($image[2]).'" alt="'.esc_attr($alt).'" '.$img_srcset_output.' class="light-dropshaddow"/></div>';
            if (trim($attachment->post_excerpt) && $caption == 'true') {
                $output .= '<div class="caption kad_caption"><div class="kad_caption_inner">' . wptexturize($attachment->post_excerpt) . '</div></div>';
            }
            if($link != 'none') { 
